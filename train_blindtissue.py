...
from utils import *
from config import BASE_DIR, PSSM_DIR, DICT_PATH, TISSUE_DICT_PATH

def get_hiv2_holdout_indices(entry_ids, tissue_map, target_tissue):
    train_idx = []
    val_idx = []
    
    print(f"\n[Split Strategy] Specific Hold-out: Test on '{target_tissue}'")
    
    for i, eid in enumerate(entry_ids):
        eid_str = str(eid)
        
        tissue = tissue_map.get(eid_str)
...
def main():
    ...
    ap.add_argument("--out_dir", type=str, default="ckpt_hiv2_holdout")
    ...
    ap.add_argument("--tissue", type=str, default="Human immunodeficiency virus (HIV-1)", help="Target tissue for hold-out")
    ...
    tissue_map = None
    if os.path.exists(TISSUE_DICT_PATH):
        print(f"Loading Tissue Dictionary from: {TISSUE_DICT_PATH}")
        with open(TISSUE_DICT_PATH, 'r') as f:
            tissue_map = json.load(f)
    else:
        raise FileNotFoundError(f"Error: Tissue dictionary not found at {TISSUE_DICT_PATH}. Run your previous script to generate it.")
    ...
    folds = get_hiv2_holdout_indices(entry_ids, tissue_map, args.tissue)

    
    for fold_id, (train_idx, val_idx) in enumerate(folds):
        print(f"\n===== {args.tissue.upper()} HOLDOUT EXPERIMENT | train={len(train_idx)} test({args.tissue})={len(val_idx)} =====")
        fold_dir = os.path.join(args.out_dir, f"{args.tissue.lower().replace(' ', '_')}_holdout")
        ...
        for m in range(args.k):
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                print(f"[Ep {epoch:03d}] Loss={tr['loss']:.4f} | {args.tissue} Test: RMSE={current_rmse:.4f} P={current_pearson:.4f}")
                ...
                if is_best:
                    ...
                    if patience >= args.early_patience:
                        print(f"  -> Early stop. Best {args.tissue} {args.metric}={best_score:.4f}")
                        break

            print(f"Member {m:02d} finished. Best {args.tissue} {args.metric}={best_score:.4f}")
        ...
        print(f"\n[FINAL {args.tissue.upper()} ENSEMBLE RESULTS]")
        print(f"Global ({args.tissue}): RMSE={ens_results['global']['rmse']:.4f} P={ens_results['global']['pearson']:.4f}")
